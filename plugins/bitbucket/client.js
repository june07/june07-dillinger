...
var Bitbucket = (function() {
  ...
  function _extractMdFiles(repo, treefiles) {
    ...
    size: 110
    type: "blob"
    url: "https://api.bitbucket.com/repos/joemccann/express/git/blobs/7a1aeb2497018aeb0c44e220d4b84f2d245e3033"
    */
    // https://raw.bitbucket.com/joemccann/express/master/History.md

    var sorted = []
      , raw = 'https://raw.bitbucket.com'
      , slash = '/'
      , ghRegex = /https:\/\/api.bitbucket.com\/(.*?)\/(.*?)\/(.*?)\/(.*)/i
      ;
    ...
    treefiles.forEach(function(el) {
      ...
      if (_isFileExt(el.path)) {
        ...
        if (Bitbucket.isRepoPrivate) {
          fullpath = el.url
        }
        else {
          // we go straight to raw as it's faster (don't need to base64 decode the sha as in the private case)
          fullpath = raw + slash + owner + slash + repo + slash + Bitbucket.currentBranch + slash + el.path
        }
  ...
  function _listOrgs(orgs) {
    ...
    var list = '<ul>' +
      '<li data-org-name="' + bitbucketUser + '" data-user="true"><a class="org" href="#">' + bitbucketUser + '</a></li>'
    ;
    ...
    $('.modal-header h3').text('Your Bitbucket Orgs')
  ...
  function _listRepos(repos) {
    ...
    var list = '<li class="bitbucket_org"><a href="#">Back to organizations...</a></li>'
      , pagination = '<ul class="repos pager"><li class="previous' + (!Bitbucket.currentPage || Bitbucket.currentPage === 1 ? " disabled" : "") + '"><a href="#">&larr; Prev</a></li><li class="next"><a href="#">Next &rarr;</a></li></ul>'
    ...
    $('.modal-header h3').text(Bitbucket.currentOwner)
  ...
  function _listBranches(repo, branches) {
    ...
    var list = '<li class="bitbucket_repo"><a href="#">Back to repositories...</a></li>'
    ...
    $('.modal-header h3').text(Bitbucket.currentOwner + " > " + repo)
  ...
  function _listTreeFiles(repo, branch, sha, treefiles) {
    ...
    $('.modal-header h3').text(Bitbucket.currentOwner + " > " + repo + " > " + Bitbucket.currentBranch)
  ...
  return {
    ...
    fetchRepos: function(owner, pager) {
      ...
      function _doneHandler(a, b, response) {
        ...
        if (!response.length) { Notifier.showMessage('No repos available!') }
          ...
          if (pager === 'next') {
            Bitbucket.currentPage++;
          }
          if (pager === 'prev') {
            Bitbucket.currentPage--;
          }

          if (Bitbucket.currentPage <= 1) {
      ...
      if (pager === 'next') {
        page = Bitbucket.currentPage + 1
      }
      else if (pager === 'prev') {
        page = Bitbucket.currentPage - 1
      }
      else {
        page = Bitbucket.currentPage
      }
    ...
    fetchTreeFiles: function(owner, repo, branch, sha) {
      ...
      , url: '/import/bitbucket/tree_files'
      , beforeSend: _beforeSendHandler
      , error: _failHandler
      , success: _doneHandler
      }
    ...
    clear: function() {
      delete profile.bitbucket.current_uri;
      delete profile.bitbucket.opts;
    },
    setInfo: function(uri, opts) {
      profile.bitbucket.current_uri = uri;
      profile.bitbucket.opts = opts;
    },
    getUri: function() {
      return profile.bitbucket.current_uri;
    },
    save: function() {
      ...
      function _doneHandler(a, b, res) {
        ...
        if (res.status < 400) {
          profile.bitbucket.opts.sha = data.content.sha
          Notifier.showMessage(Notifier.messages.docSavedBitbucket + " as " + data.content.path);
        } else {
      ...
      var postData = {
        uri: Bitbucket.getUri() || ""
      , data: btoa(editor.getSession().getValue())
      , name: profile.bitbucket.opts.name
      , sha: profile.bitbucket.opts.sha
      , branch: profile.bitbucket.opts.branch
      , repo: profile.bitbucket.opts.repo
      , owner: profile.bitbucket.opts.owner
